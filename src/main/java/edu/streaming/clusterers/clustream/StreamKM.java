...
import edu.streaming.clusterers.AbstractClusterer;
import edu.streaming.clusterers.streamkm.BucketManager;
import edu.streaming.clusterers.streamkm.MTRandom;
import edu.streaming.clusterers.streamkm.Point;
...
public class StreamKM /* extends AbstractClusterer */{
	...
	// 'w', "Size of Window for training learner.", 100000, 0,
	// Integer.MAX_VALUE);
	//
	...
	public void resetLearningImpl() {
		...
		this.coresetsize = 1000;// sizeCoresetOption.getValue();
		this.numberOfCentres = 10;// numClustersOption.getValue();
		this.length = 5;// widthOption.getValue();
		this.centresStreamingCoreset = new Point[this.numberOfCentres];

		// initalize random generator with seed
		this.clustererRandom = new MTRandom(1);
	}
	...
	public void trainOnInstanceImpl(/*Instance inst*/) {
		...
		if (this.initialized == false) {
			this.dimension = numattributes;
			manager = new BucketManager(this.length, this.dimension,
					this.coresetsize, this.clustererRandom);
		...
		manager.insertPoint(new Point(X, this.numberInstances));

		this.numberInstances++;
		if (this.numberInstances % width == 0) {

			Point[] streamingCoreset = manager.getCoresetFromManager(dimension);

			// compute 5 clusterings of the coreset with kMeans++ and take the
			// best
			double minCost = 0.0;
			double curCost = 0.0;

			minCost = lloydPlusPlus(numberOfCentres, coresetsize, dimension,
					streamingCoreset, centresStreamingCoreset);
			curCost = minCost;

			for (int i = 1; i < 5; i++) {
				Point[] tmpCentresStreamingCoreset = new Point[0];
				curCost = lloydPlusPlus(numberOfCentres, coresetsize,
						dimension, streamingCoreset, tmpCentresStreamingCoreset);
				if (curCost < minCost) {
					minCost = curCost;
...
//	}
	...
	public double lloydPlusPlus(int k, int n, int d, Point points[],
		...
		/*
		 * printf("Centres: \n"); int i=0; for(i=0;i<k;i++){ printf("("); int l
		 * = 0; for(l=0;l<centres[i].dimension;l++){
	...
	private Point[] chooseRandomCentres(int k, int n, int d, Point points[]) {
		 ...
		 * //printf("%d: (",i); int l = 0; for(l = 0; l < centres[i].dimension;
		 * l++){ printf("%f,",centres[i].coordinates[l] / centres[i].weight); }
		 * printf(")\n"); }
		 */
	...
	/**
	 * computes the target function for the given pointarray points[] (of size
	 * n) with the given array of centres centres[] (of size k)
	 **/
	public double targetFunctionValue(int k, int n, Point[] centres,
			Point[] points) {
		int i = 0;
		double sum = 0.0;
		for (i = 0; i < n; i++) {
